...
using  Matriz_Distancia = std::vector < vector <int> >;
...
void lectura_distancia(string filename)
{
	...
	for (int i = 0; i < Matriz_Distancia.size(); i++) {
		for (int j = 0; j < Matriz_Distancia[i].size(); j++)
		{
			cout << "Dato : " << i << j << endl;
	...
	for (int i = 0; i < Matriz_Distancia.size(); i++) {
		for (int j = 0; j < Matriz_Distancia[i].size(); j++)
		{
			...
			if (i <4 && j<4)
			{
				...
				distancia = distancia + Matriz_Distancia[i][j];
...
int main()
{
	...
	string filename = "C:\\Users\\Tbusiness\\Source\\Repos\\Entrega2\\Entrega2\\Distancia.txt";
	lectura_distancia(filename);
	Nodo_1 nodo;

		int distancia = 0;
		
		Matriz_Distancia matriz_aux	;
	int demanda = 0;
	for (int i = 0; i < matriz_aux.size(); i++) {
		for (int j = 0; j < matriz_aux[i].size(); j++)
		{
			...
			if (i < 4 && j < 4)
			{
				...
				distancia = distancia + matriz_aux[i][j];
				cout << "Distancia recorrida en  " << i << j << " :" << distancia << endl;
			}
		}
