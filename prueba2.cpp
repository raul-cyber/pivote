...
int main()
{
	...
	int demanda = 0;
        Nodo_1 nodo;
        int movimiento[Nodo.size()][2];
        int ocupacion[Vehiculos.size()][Nodo.size()];
        int x,y;
        x = 0;
// matriz para movimientos entre nodos
        for(auto nodo = Nodo.begin(); nodo != Nodo.end(); nodo++){

            if(nodo->second.clasi == 0){
		movimiento [x][0] = 1;
                movimiento [x][1] = 1;
	     }
	     if(nodo->second.clasi == 1){
		movimiento [x][0] = 0;
                movimiento [x][1] = 1;
	     }
...
// matriz de ocupacion de vehiculo en nodo
        x=0;
        y=0;

	for(auto nodo = Nodo.begin(); nodo != Nodo.end(); nodo++){

	   for(auto vehiculo = Vehiculos.begin(); vehiculo != Vehiculos.end(); vehiculo++){
		if(nodo->second.demanda <= vehiculo->second.capacidad){
                    ocupacion[x][y] = 1;
		 }else{
		      ocupacion[x][y] = 0;
		  }
	...
	for(int i =0 ; i < Nodo.size() ; i++){
		for(int j =0 ; j < Vehiculos.size(); j++){
                   cout << i << ","<< j << " "<< ocupacion[i][j] << endl;
		}
	}
}


// Cambiar la matriz de ocupacion por un map int, matriz  donde se almacenandn los posibles nodos que recorre  el vehiculo
// agregar la validacion de poder ir desde el nodo actual al siguiente utilizando la matriz de movimiento
